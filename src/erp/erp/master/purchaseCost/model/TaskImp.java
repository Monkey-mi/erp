package erp.erp.master.purchaseCost.model;

import java.util.Date;

public class TaskImp {
	private double			rwdh;
	private double			rwxh;
	private double			jhbh;
	private double			jhxh;
	private String			cpbh;
	private String			wxh;
	private String			jhh;
	private String			zyh;
	private String			zjh;
	private String			jldw;
	private double			tcsl;
	private double			jsbl;
	private double			rwsl;
	private double			yrsl;
	private double			drsl;
	private double			jgje;
	private double			wcsl;
	private int			wcbj;
	private String			bzsm;
	private int			sjbj;
	private double			zybh;
	private double			zyxh;
	private double			bzgj;
	private Date			sjrq;
	private Date			wcrq;
	private double			gjxs;
	private double			jsdj;
	private int			jsbj;
	private String			jsrm;
	private Date			jssj;
	private double			psmj;
	private String			fzhm;
	private double			fcsl;
	private double			sqbh;
	private double			sqxh;
	private String			flbz;
	private double			jssl;
	private int			zzbj;
	private int			kjbj;
	private String			zzrm;
	private String			csmc;
	private String			cpmc;
	private String			gxbh;
	private String			jhlb;
	private String			plmth;
	private String			plmtx;
	private String			zcpbh;
	private String			zcpmc;
	private String			jhlbmc;
	private String			khmc;
	private String			jhbz;
	private String			cpth;
	private String			thbh;
	private String			gxmc;
	private String			csbh;
	private Date			zzrq;
	private double			zhdj;
	private double			fysl;
    
	public String getCsbh() {
		return csbh;
	}
	public void setCsbh(String csbh) {
		this.csbh = csbh;
	}
	public String getGxmc() {
		return gxmc;
	}
	public void setGxmc(String gxmc) {
		this.gxmc = gxmc;
	}
	public String getZcpmc() {
		return zcpmc;
	}
	public void setZcpmc(String zcpmc) {
		this.zcpmc = zcpmc;
	}
	public String getJhlbmc() {
		return jhlbmc;
	}
	public void setJhlbmc(String jhlbmc) {
		this.jhlbmc = jhlbmc;
	}
	public int getKjbj() {
		return kjbj;
	}
	public void setKjbj(int kjbj) {
		this.kjbj = kjbj;
	}
	public String getCsmc() {
		return csmc;
	}
	public void setCsmc(String csmc) {
		this.csmc = csmc;
	}
	public String getCpmc() {
		return cpmc;
	}
	public void setCpmc(String cpmc) {
		this.cpmc = cpmc;
	}
	public String getGxbh() {
		return gxbh;
	}
	public void setGxbh(String gxbh) {
		this.gxbh = gxbh;
	}
	public String getJhlb() {
		return jhlb;
	}
	public void setJhlb(String jhlb) {
		this.jhlb = jhlb;
	}
	public String getPlmth() {
		return plmth;
	}
	public void setPlmth(String plmth) {
		this.plmth = plmth;
	}
	public String getPlmtx() {
		return plmtx;
	}
	public void setPlmtx(String plmtx) {
		this.plmtx = plmtx;
	}
	public String getZcpbh() {
		return zcpbh;
	}
	public void setZcpbh(String zcpbh) {
		this.zcpbh = zcpbh;
	}
	public String getKhmc() {
		return khmc;
	}
	public void setKhmc(String khmc) {
		this.khmc = khmc;
	}
	public String getJhbz() {
		return jhbz;
	}
	public void setJhbz(String jhbz) {
		this.jhbz = jhbz;
	}
	public String getCpth() {
		return cpth;
	}
	public void setCpth(String cpth) {
		this.cpth = cpth;
	}
	public String getThbh() {
		return thbh;
	}
	public void setThbh(String thbh) {
		this.thbh = thbh;
	}
	public String getWxh() {
		return wxh;
	}
	public void setWxh(String wxh) {
		this.wxh = wxh;
	}
	public String getJhh() {
		return jhh;
	}
	public void setJhh(String jhh) {
		this.jhh = jhh;
	}
	public String getZyh() {
		return zyh;
	}
	public void setZyh(String zyh) {
		this.zyh = zyh;
	}
	public String getZjh() {
		return zjh;
	}
	public void setZjh(String zjh) {
		this.zjh = zjh;
	}
	public double getYrsl() {
		return yrsl;
	}
	public void setYrsl(double yrsl) {
		this.yrsl = yrsl;
	}
	public double getDrsl() {
		return drsl;
	}
	public void setDrsl(double drsl) {
		this.drsl = drsl;
	}
	public double getJgje() {
		return jgje;
	}
	public void setJgje(double jgje) {
		this.jgje = jgje;
	}
	public double getRwdh() {
		return rwdh;
	}
	public void setRwdh(double rwdh) {
		this.rwdh = rwdh;
	}
	public double getRwxh() {
		return rwxh;
	}
	public void setRwxh(double rwxh) {
		this.rwxh = rwxh;
	}
	public double getJhbh() {
		return jhbh;
	}
	public void setJhbh(double jhbh) {
		this.jhbh = jhbh;
	}
	public double getJhxh() {
		return jhxh;
	}
	public void setJhxh(double jhxh) {
		this.jhxh = jhxh;
	}
	public String getCpbh() {
		return cpbh;
	}
	public void setCpbh(String cpbh) {
		this.cpbh = cpbh;
	}
	public String getJldw() {
		return jldw;
	}
	public void setJldw(String jldw) {
		this.jldw = jldw;
	}
	public double getTcsl() {
		return tcsl;
	}
	public void setTcsl(double tcsl) {
		this.tcsl = tcsl;
	}
	public double getJsbl() {
		return jsbl;
	}
	public void setJsbl(double jsbl) {
		this.jsbl = jsbl;
	}
	public double getRwsl() {
		return rwsl;
	}
	public void setRwsl(double rwsl) {
		this.rwsl = rwsl;
	}
	public double getWcsl() {
		return wcsl;
	}
	public void setWcsl(double wcsl) {
		this.wcsl = wcsl;
	}
	public int getWcbj() {
		return wcbj;
	}
	public void setWcbj(int wcbj) {
		this.wcbj = wcbj;
	}
	public String getBzsm() {
		return bzsm;
	}
	public void setBzsm(String bzsm) {
		this.bzsm = bzsm;
	}
	public int getSjbj() {
		return sjbj;
	}
	public void setSjbj(int sjbj) {
		this.sjbj = sjbj;
	}
	public double getZybh() {
		return zybh;
	}
	public void setZybh(double zybh) {
		this.zybh = zybh;
	}
	public double getZyxh() {
		return zyxh;
	}
	public void setZyxh(double zyxh) {
		this.zyxh = zyxh;
	}
	public double getBzgj() {
		return bzgj;
	}
	public void setBzgj(double bzgj) {
		this.bzgj = bzgj;
	}
	public Date getSjrq() {
		return sjrq;
	}
	public void setSjrq(Date sjrq) {
		this.sjrq = sjrq;
	}
	public Date getWcrq() {
		return wcrq;
	}
	public void setWcrq(Date wcrq) {
		this.wcrq = wcrq;
	}
	public double getGjxs() {
		return gjxs;
	}
	public void setGjxs(double gjxs) {
		this.gjxs = gjxs;
	}
	public double getJsdj() {
		return jsdj;
	}
	public void setJsdj(double jsdj) {
		this.jsdj = jsdj;
	}
	public int getJsbj() {
		return jsbj;
	}
	public void setJsbj(int jsbj) {
		this.jsbj = jsbj;
	}
	public String getJsrm() {
		return jsrm;
	}
	public void setJsrm(String jsrm) {
		this.jsrm = jsrm;
	}
	public Date getJssj() {
		return jssj;
	}
	public void setJssj(Date jssj) {
		this.jssj = jssj;
	}
	public double getPsmj() {
		return psmj;
	}
	public void setPsmj(double psmj) {
		this.psmj = psmj;
	}
	public String getFzhm() {
		return fzhm;
	}
	public void setFzhm(String fzhm) {
		this.fzhm = fzhm;
	}
	public double getFcsl() {
		return fcsl;
	}
	public void setFcsl(double fcsl) {
		this.fcsl = fcsl;
	}
	public double getSqbh() {
		return sqbh;
	}
	public void setSqbh(double sqbh) {
		this.sqbh = sqbh;
	}
	public double getSqxh() {
		return sqxh;
	}
	public void setSqxh(double sqxh) {
		this.sqxh = sqxh;
	}
	public String getFlbz() {
		return flbz;
	}
	public void setFlbz(String flbz) {
		this.flbz = flbz;
	}
	public double getJssl() {
		return jssl;
	}
	public void setJssl(double jssl) {
		this.jssl = jssl;
	}
	public int getZzbj() {
		return zzbj;
	}
	public void setZzbj(int zzbj) {
		this.zzbj = zzbj;
	}
	public String getZzrm() {
		return zzrm;
	}
	public void setZzrm(String zzrm) {
		this.zzrm = zzrm;
	}
	public Date getZzrq() {
		return zzrq;
	}
	public void setZzrq(Date zzrq) {
		this.zzrq = zzrq;
	}
	public double getZhdj() {
		return zhdj;
	}
	public void setZhdj(double zhdj) {
		this.zhdj = zhdj;
	}
	public double getFysl() {
		return fysl;
	}
	public void setFysl(double fysl) {
		this.fysl = fysl;
	}
}
