...
Ext.define('erp.basic.controller.BasicData',{
	extend: 'Ext.app.Controller',
	requires: ['erp.def.Const',
	           'erp.util.Util',
	           'erp.def.ui.plugins.FormKeyMapper',
	           'erp.ux.PagingBar',
	           'erp.basic.store.Codes',
		       'erp.basic.store.CodeTypes'],
	models:['erp.basic.model.Code'],
	views: ['erp.basic.view.MngBasicData',
	        'erp.basic.view.EditCodeType',
	        'erp.basic.view.EditCode'
	        ],
    refs: [
    ...
    init: function(){
		...
		var me = this;
		me.type_code=Ext.create('erp.basic.store.CodeTypes',{
			proxy : {
				type : 'ajax',
				actionMethods : 'post',
				api : {
	...
	doAddCodeType: function(){
		var rec = Ext.create('erp.basic.model.CodeType',{
			attrib:erp.Const.TYPE_ATTRIB_CBX
		});
	...
	doAddCode : function() {
		...
		var selModel = this.getCodeTypeGrid().getSelectionModel();
		rec = selModel.getSelection()[0];
		var record = Ext.create('erp.basic.model.Code', {
			type_code : rec.get('type_code')
		});
		edtWin = Ext.create('erp.basic.view.EditCode', {
					isAddNew : true
				});
	...
	doSaveCodeConfig:function(rec){
		var me=this;
		var model=Ext.create('erp.basic.model.CodeConfig',{
			name:rec.get('name'),
			type:erp.DataConst.DICTIONARY,
			code_type:rec.get('type_code'),
			displayField:'name',
			valueField:'value'
		});
		var store=erp.DataUtil.getStoreByStoreManager(erp.DataConst.CODECONFIG_STOREID,true);
		
		erp.DataUtil.sync(store,model);
	},
