Ext.define('erp.basicdata.tradeClass.view.TradeClassManager',{	
    ...
    initComponent:function(){
    	var me =this;
    	me.store=Ext.create('erp.basicdata.tradeClass.store.TradeClass');
 		Ext.apply(me.store.proxy.extraParams, {usePaging:true});
 		
 		me.treeStore=Ext.create('erp.basicdata.tradeClass.store.TradeClassTree');
 		var rowEditing=Ext.create('Ext.grid.plugin.RowEditing', {
			        ...
			        listeners: {
						...
						'validateedit':function(editor,e,obj){
							if(e.record.get('oo')=='000'){			        							
								var rec=e.record;
								var result=erp.Const.callServiceMethodSync('tradeClass/tradeClass.srm?method=checkForAdd',{
			        			...
			        			var rec=e.record;
								var result=erp.Const.callServiceMethodSync('tradeClass/tradeClass.srm?method=checkForUpdate',{
   	  	  	...
   	  	  	dockedItems:[
				  		...
				  		{text: '新增',	glyph : 0xf016,		itemId:erp.Const.FUNC_ITEMID_BTN_ADD
				  			,handler:function(){
								...
								var rec=Ext.create('erp.basicdata.tradeClass.model.TradeClass',{
									oo:'000',
									f_id:parentId
								});
 	...
 	doDelete: function() {
		...
		var result=erp.Const.callServiceMethodSync('tradeClass/tradeClass.srm?method=candeleted',{
				class_id:rec.get('class_id')
		});
		var result=Ext.decode(result);  
