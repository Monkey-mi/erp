Ext.define('erp.basicdata.area.view.AreaManager',{	
	extend:'erp.ux.Panel',
    alias:'widget.areaManager',
    
    initComponent:function(){
    	var me =this;
    	me.store=Ext.create('erp.basicdata.area.store.Area');
 		Ext.apply(me.store.proxy.extraParams, {usePaging:true});
 		
 		me.treeStore=Ext.create('erp.basicdata.area.store.AreaTree');
 		var rowEditing=Ext.create('Ext.grid.plugin.RowEditing', {
			        ...
			        listeners: {
						...
						'validateedit':function(editor,e,obj){
							if(e.record.get('oo')=='000'){			        							
								var rec=e.record;
								var result=erp.Const.callServiceMethodSync('area/area.srm?method=checkForAdd',{
			        			...
			        			var rec=e.record;
								var result=erp.Const.callServiceMethodSync('area/area.srm?method=checkForUpdate',{
										area_name:e.newValues.area_name,
										f_id:rec.get('f_id'),
   	  	  	...
   	  	  	dockedItems:[
				  		...
				  		{text: '新增',	glyph : 0xf016,		itemId:erp.Const.FUNC_ITEMID_BTN_ADD
				  			,handler:function(){
								...
								var rec=Ext.create('erp.basicdata.area.model.Area',{
									oo:'000',
									f_id:parentId
								});
 	...
 	doDelete: function() {
		...
		var result=erp.Const.callServiceMethodSync('area/area.srm?method=candeleted',{
				area_id:rec.get('area_id')
		});
		var result=Ext.decode(result);  
