Ext.define('erp.basicdata.level.view.MaterialLevelManager',{	
    ...
    initComponent:function(){
    	var me =this;
    	me.store=Ext.create('erp.basicdata.level.store.MaterialLevel');
 		...
 		var rowEditing=Ext.create('Ext.grid.plugin.RowEditing', {
			        ...
			        listeners: {
						...
						'validateedit':function(editor,e,obj){
							if(e.record.get('oo')=='000'){			        							
								var rec=e.record;
								var result=erp.Const.callServiceMethodSync('materialLevel/materialLevel.srm?method=checkForAdd',{
			        			...
			        			var rec=e.record;
								var result=erp.Const.callServiceMethodSync('materialLevel/materialLevel.srm?method=checkForUpdate',{
										level_name:e.newValues.level_name,
   	  	  	...
   	  	  	dockedItems:[
				  		...
				  		{text: '新增',	glyph : 0xf016,		itemId:erp.Const.FUNC_ITEMID_BTN_ADD
				  			,handler:function(){
								...
								var rec=Ext.create('erp.basicdata.level.model.MaterialLevel',{
									oo:'000'
								});
 	...
 	doDelete: function() {
		...
		var result=erp.Const.callServiceMethodSync('materialLevel/materialLevel.srm?method=candeleted',{
				level_id:rec.get('level_id')
		});
		var result=Ext.decode(result);  
