Ext.define('erp.setup.controller.Module', {
	extend : 'Ext.app.Controller',
	requires : [ 'erp.def.Const', 'erp.util.Util' ],
	models : [ 'erp.setup.model.Module', 'erp.setup.model.ModuleTree' ],
	views : [ 'erp.setup.view.MngModule', 'erp.setup.view.EditModule' ],
	refs : [
		...
		this.control({
			'mng_Module' : {
				...
				afterrender : function() {
					this.changeStore = Ext.create('erp.setup.store.Modules');
	...
	onGridItemdbclik : function(view, rec) {
		...
		if (!this.modFuncsDisabled[erp.Const.FUNC_ITEMID_BTN_EDT])
	...
	doAddModule : function() {
		...
		var rec = Ext.create('erp.setup.model.Module');
	...
	doDelModule : function() {
		...
		var rec = selModel.getSelection()[0];
		Ext.Ajax.request({
			url : 'main/Modules.do?method=getCountByPId',
			params : {
				node : rec.get('id')
			},
