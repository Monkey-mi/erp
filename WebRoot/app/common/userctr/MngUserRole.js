 ...
 * 用户角色关系界面
 */
Ext.define('erp.common.userctr.MngUserRole',{
    extend:'erp.ux.Panel',
    ...
    roleReset:function(){
        var me=this;
        //提取可供授权的角色
        me.roleStore = erp.DataUtil.getStoreByStoreManager(erp.DataConst.ROLE);
        Ext.Ajax.request({
           url:'main/Users.do?method=getCanChooseRoleList',
           async:false,
           params:{
              u_id : me.currentUser.get('u_id'),
              ou_id : me.ou_id
        ...
        Ext.each(me.roleStore.getRange(),function(rec){
         if(me.SelectedStore.find('role_id',rec.get('role_id'),0,false,false,true)<0){
         	me.unSelectStore.add(Ext.create('erp.user.model.UserRole',{
              u_id:me.currentUser.get('u_id'),
    ...
    initComponent:function(){
    	...
    	me.unSelectStore=Ext.create('erp.user.store.UserRoles');
    	me.SelectedStore=Ext.create('erp.user.store.UserRoles');
    	Ext.apply(me,{
    	items:[
    	{
    		xtype:'grid',
    		...
    		columns:[
    		{
    			...
    			renderer:function(v){
    				if(me.allRoles){
        				var role_name=me.allRoles.findRecord("role_id",v,0,false,false,true).get('role_name');
        				return role_name;
    				}else
    				    return v;
    			}
