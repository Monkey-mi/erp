Ext.define('erp.supplier.controller.SupplierCtrl', {
	...
	requires : [
				...
				,'erp.supplier.store.CompanyClass'
				,'erp.supplier.store.Attched'
				,'erp.supplier.store.CustomAttched'
				,'erp.supplier.store.InvoiceTitle'
				,'erp.supplier.store.Competitor'
				,'erp.supplier.store.BankAccount'
				,'erp.supplier.store.MainCustomer'
				,'erp.supplier.store.Devicelist'
				,'erp.supplier.store.Metarial'
				,'erp.supplier.store.Goods'
				,'erp.supplier.store.Area'
				,'erp.supplier.store.MaterialClassTree'
				,'erp.supplier.store.MaterialClass'
				],
	views : [
				'erp.supplier.view.SupplierManager',
				'erp.supplier.view.UpAttchedFile',
				'erp.supplier.view.MaterialClassTreeWin'
			],
	refs : [
	...
	doEditAction:function(btn){
		...
		switch(btn.itemId){
				...
				if(me.store.indexOf(rec)>= 0&&rec.get('company_id')>0){//update
					...
					if (form.getForm().isDirty()&&form.getForm().isValid()){
						me.store.sync({
							success: function(batch,options) {	
								erp.UInfo.currentUser.supplier=rec;
							}
						});
					}
					//保存详细数据
	    			...
	    			me.store.sync({
						success : function(e, batch) {
							 ...
							 erp.UInfo.currentUser.supplier=newRec;//重新给供应商对象赋值
							 //设置user所属的供应商
							 erp.UInfo.currentUser.userInfo.company_id=company_id;//更新本地用户信息
							 //更新数据库用户信息
							 erp.Const.callServiceMethodSync('common/update_company_id.do',{
									company_id:company_id,
									cpyname_cn:cpyname_cn,
									u_id:erp.UInfo.currentUser.userInfo.u_id
							 });
							 //保存详细数据
							 me.saveDetail(company_id);
						}
			...
			case 'BTN_ADD_plinvoice':
				var newrec=Ext.create('erp.supplier.model.InvoiceTitle');
...
//				break;
			case 'BTN_ADD_plcompetitor':
				var newrec=Ext.create('erp.supplier.model.Competitor');
			...
			case 'BTN_DEL_plcompetitor':
				...
				break;
			case 'BTN_ADD_plbank':
				var newrec=Ext.create('erp.supplier.model.BankAccount');
...
//				var newrec=Ext.create('erp.supplier.model.Attched',{
//					create_dt:new Date()
//				});
...
//				}
			...
			case 'BTN_ADD_customer':
				var newrec=Ext.create('erp.supplier.model.MainCustomer');
			...
			case 'BTN_ADD_plEquipment':
				var newrec=Ext.create('erp.supplier.model.Devicelist');
				me.devicestore.insert(me.devicestore.getCount(),newrec);
				break;
			case 'BTN_DEL_plEquipment':
				var grid_device=me.mainview.down('#plEquipmentDetails');
			...
			case 'BTN_ADD_plmaterials':
				var newrec=Ext.create('erp.supplier.model.Metarial');
			...
			case 'BTN_ADD_plproduct':
				var newrec=Ext.create('erp.supplier.model.Goods');
	...
	checkData:function(){
		...
		if(!form.getForm().isValid()){
			...
			return flag;
			 ...
			 var count=erp.Const.callServiceMethodSync('supplier/supplierFile.do?method=getCountByIdAndName',{
...
//				me.mainview.deleteUploadFile();
	...
	checkStatusSame:function(sel_rec){
		...
		var json=erp.Const.callServiceMethodSync('supplier/supplierFile.do?method=checkStatusSame',{
			company_id:company_id,
		...
		if(json.result==0){
			me.store.load({
				...
				callback: function(records, operation, success) {
					...
					erp.UInfo.currentUser.supplier=records[0];
