Ext.define('erp.supplier.view.SupplierManager', {
	extend : 'erp.ux.Panel',
	alias : 'widget.supplierSelfManager',
	requires:[
			  'erp.basicdata.taxmanClass.store.TaxmanClass',
			  'erp.basicdata.industryClass.store.IndustryClass'],
	initComponent : function() {
		var me=this;
		me.cannotedit=true;//默认不可编辑，在disabledForm中改变
		me.store=Ext.create('erp.supplier.store.SupplierFile');
		me.ccstore=Ext.create('erp.supplier.store.CompanyClass');
		me.ccstore.load();
		//公司性质之行业性质
		me.industryClassStore=Ext.create('erp.basicdata.industryClass.store.IndustryClass');
		me.industryClassStore.load();
		//公司性质之纳税人类别
		me.taxmanClassStore=Ext.create('erp.basicdata.taxmanClass.store.TaxmanClass');
		me.taxmanClassStore.load();
		//附件
		me.attchedstore=Ext.create('erp.supplier.store.Attched');
		//自定义附件
		me.customAttchedStore=Ext.create('erp.supplier.store.CustomAttched');
		me.invoicestore=Ext.create('erp.supplier.store.InvoiceTitle');
		me.competitorstore=Ext.create('erp.supplier.store.Competitor');
		me.bankstore=Ext.create('erp.supplier.store.BankAccount');
		me.customerstore=Ext.create('erp.supplier.store.MainCustomer');
		me.devicestore=Ext.create('erp.supplier.store.Devicelist');
		me.metarialstore=Ext.create('erp.supplier.store.Metarial');
		me.goodsstore=Ext.create('erp.supplier.store.Goods');
		me.first_areastore=Ext.create('erp.supplier.store.Area');
		me.first_areastore.load({
			params:{
				levelType:1
			}
		});
		me.reg_second_areastore=Ext.create('erp.supplier.store.Area');
		me.reg_third_areastore=Ext.create('erp.supplier.store.Area');
		me.contact_second_areastore=Ext.create('erp.supplier.store.Area');
		me.contact_third_areastore=Ext.create('erp.supplier.store.Area');
		me.materialClassstore=Ext.create('erp.supplier.store.MaterialClass');
		me.treeStore=Ext.create('erp.supplier.store.MaterialClassTree');
				...
				,{
					...
					items:[
					{
						...
						handler:function(grid,rowIndex,colIndex){
							var supplier_rec=erp.UInfo.currentUser.supplier;
...
//                			}
	...
	loadData:function(){
		...
		var company_id;
		if(!Ext.isEmpty(erp.UInfo.currentUser.supplier)){
			me.store.load({
				params:{
					company_id:erp.UInfo.currentUser.supplier.get('company_id')
				},
			...
			rec=Ext.create('erp.supplier.model.SupplierFile');
	...
	setDefaultAttchedData:function(company_id){
		...
		var array=[];
			var create_dt=new Date();
			var file1=Ext.create('erp.supplier.model.Attched',{company_id:company_id,file_name:'营业执照',create_dt:create_dt,file_format:'jpg/jpeg/png/gif/bmp',ismust:true,remark:'请上传图片格式，如jpg,jpeg,png,gif,bmp',iscustom:false});
			var file2=Ext.create('erp.supplier.model.Attched',{company_id:company_id,file_name:'税务登记证',create_dt:create_dt,file_format:'jpg/jpeg/png/gif/bmp',ismust:true,remark:'请上传图片格式，如jpg,jpeg,png,gif,bmp',iscustom:false});
			var file3=Ext.create('erp.supplier.model.Attched',{company_id:company_id,file_name:'组织机构代码证',create_dt:create_dt,file_format:'jpg/jpeg/png/gif/bmp',ismust:true,remark:'请上传图片格式，如jpg,jpeg,png,gif,bmp',iscustom:false});
			var file4=Ext.create('erp.supplier.model.Attched',{company_id:company_id,file_name:'公司介绍',create_dt:create_dt,file_format:'word/pdf/excel/rar/zip',ismust:false,remark:'文件大小不得超过10M',iscustom:false});
			var file5=Ext.create('erp.supplier.model.Attched',{company_id:company_id,file_name:'厂容厂貌',create_dt:create_dt,file_format:'jpg/jpeg/png/gif/bmp',ismust:false,remark:'请上传图片格式，如jpg,jpeg,png,gif,bmp',iscustom:false});
			var file6=Ext.create('erp.supplier.model.Attched',{company_id:company_id,file_name:'认证体系证书复印件',create_dt:create_dt,file_format:'jpg/jpeg/png/gif/bmp',ismust:false,remark:'文件大小不得超过10M',iscustom:false});
			var file7=Ext.create('erp.supplier.model.Attched',{company_id:company_id,file_name:'公司/工厂机器设备清单',create_dt:create_dt,file_format:'word/pdf/excel/rar/zip',ismust:false,remark:'文件大小不得超过10M',iscustom:false});
			var file8=Ext.create('erp.supplier.model.Attched',{company_id:company_id,file_name:'代理商/贸易商需提供代理证明资质',create_dt:create_dt,file_format:'jpg/jpeg/png/gif/bmp',ismust:false,remark:'请上传图片格式，如jpg,jpeg,png,gif,bmp',iscustom:false});
			var file9=Ext.create('erp.supplier.model.Attched',{company_id:company_id,file_name:'同类产品国家认定机构检测报告或行业出具的等级证明、优质产品证书、知识产权证书复印件',create_dt:create_dt,file_format:'jpg/jpeg/png/gif/bmp',ismust:false,remark:'请上传图片格式，如jpg,jpeg,png,gif,bmp',iscustom:false});
			var file10=Ext.create('erp.supplier.model.Attched',{company_id:company_id,file_name:'与院校、科研机构合作、交流情况(提供相关文件复印件)',create_dt:create_dt,file_format:'jpg/jpeg/png/gif/bmp',ismust:false,remark:'请上传图片格式，如jpg,jpeg,png,gif,bmp',iscustom:false});
			array.push(file1);
			array.push(file2);
			array.push(file3);
			array.push(file4);
