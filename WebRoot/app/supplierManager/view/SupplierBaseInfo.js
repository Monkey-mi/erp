//供应商基本信息页面
Ext.define('erp.supplierManager.view.SupplierBaseInfo',{
	extend:'erp.ux.Panel',
	alias:'widget.SupplierBaseInfo',
	requires:['erp.supplierManager.view.SupplierBusInfo',
			...
			'erp.supplierManager.store.Area'],
	overflowY: 'auto',
	//height:800,
//	isAdd:false,
//	isEdit:false,
	initComponent:function(){
		var me=this;
		me.companyClassStore=Ext.create('erp.basicdata.companyClass.store.CompanyClass');
		...
		me.first_areastore=Ext.create('erp.supplierManager.store.Area');
		me.first_areastore.load({
			params:{
				levelType:1
			}
		});
		me.reg_second_areastore=Ext.create('erp.supplierManager.store.Area');
		me.reg_third_areastore=Ext.create('erp.supplierManager.store.Area');
		me.contact_second_areastore=Ext.create('erp.supplierManager.store.Area');
		me.contact_third_areastore=Ext.create('erp.supplierManager.store.Area');
//		//厂商类别
...
//		me.treepickerStore.proxy.extraParams.is_archive=0;
		...
		//供应商子材料类别
		me.supplierMaterialSub1Store=Ext.create('erp.supplierManager.store.SupplierMaterialSub1');
		//主体单位
		me.mainUnitStore=Ext.create('erp.view.master.purchaseDetail.store.MainUnit');
		me.mainUnitStore.proxy.extraParams.gdbj=0;
		me.mainUnitStore.load();
		me.wbStore=Ext.create('erp.master.foreigncurrency.store.foreignCurrency');
		me.wbStore.load();
		Ext.apply(me,{
			defaults:{
				padding:0,
...
//						emptyText:'(必填项)',
						...
						listeners:{
							change:function(combo,newValue){
								...
								if(wbbh==null){
									...
									wbbh=Ext.String.trim(wbbh);
								}
								if(wbbh!=''&&wbbh!='60'){
									me.down('#wbhs').setValue(true);
								}else{
									me.down('#wbhs').setValue(false);
								}
							}
						}
	           		},
	...
	{
		var me=this;
		var list=erp.Const.callServiceMethodSync('materialClass/materialClass.srm?method=getMaterialClassListByFids',{fidArray:fidArray.join(',')});
		return list;
	}
});