//数据处理工具包
Ext.define('erp.util.data.DataUtil',{
	requires:['erp.util.data.DataManager',
	          'erp.util.data.Const',
	          'erp.setup.store.Codes',
	          'erp.common.form.store.FrmRegs',
	          'erp.common.basic.store.CodeConfigs',
	          'erp.common.form.store.FrmRenders'
	          //'erp.common.form.store.FrmTbls'
//	          'tp.basic.tree.view.GpTreeField'
    ],
	...
	init:function(){
		...
		var CBXStore;
		var codeConfig;
		var DataManager=new erp.util.data.DataManager();
		me.initProMises();
		me.initAutoStore()
		/**
		 * 向dataManager注册store
		...
		me.findStore=function(storeId,storeName){
			...
			var returnstore;
			var storeTest=Ext.create('erp.common.basic.store.CodeConfigs');
			if(!look&&storeName){
		...
		me.findConfig=function(type){
			var rec=me.getStoreByStoreManager(erp.DataConst.CODE_CONFIG).findRecord('code',type,0,false,false,true);
			return rec;
		};
		...
		me.getStoreByType=function(type){
			...
			if(rec.get('type')==erp.DataConst.DICTIONARY){
		...
		me.DataLoad=function(){
			 ...
			 */
			Ext.Ajax.request({
				url:'form/Forms.do?method=getCodeFrmList',
				method:'post',
				async:false,
				success:function(resp){
					var text=Ext.decode(resp.responseText);
					var regs=text.data.reg;
					var tbls=text.data.tbl;
					var renders=text.data.renders;
					var regStore=me.createStoreFactory("erp.common.form.store.FrmRegs",false);
					var tblStore=me.createStoreFactory("erp.common.form.store.FrmTbls", false);
					var renderStore=me.createStoreFactory("erp.common.form.store.FrmRenders",false);
					regStore.storeId=erp.DataConst.FRMREG_STOREID;
					tblStore.storeId=erp.DataConst.FRMTBL_STOREID;
					renderStore.storeId=erp.DataConst.FRMRENDER_STOREID;
					regStore.loadData(regs,true);
					tblStore.loadData(tbls,true);
					renderStore.loadData(renders,true);
					me.setStoreToStoreManager(regStore);
					me.setStoreToStoreManager(renderStore);
					Ext.each(tbls,function(rec){
						var tbl=tblStore.findRecord('ft_id',rec.ft_id,0,false,false,true);
						tbl.frmFldsStore=me.createStoreFactory("erp.common.form.store.FrmFlds", false);
						tbl.frmFldsStore.loadData(rec.frmFlds,true);
					});
					me.setStoreToStoreManager(tblStore);
				}
			});
			/**
			 * 下拉数据仓库初始化
			 */
			var codeStore=erp.DataUtil.createStoreFactory('erp.setup.store.Codes');
			codeStore.load({
				async:false,
				params:{
					type_attrib:erp.Const.TYPE_ATTRIB_CBX
				},
				callback:function(){
					CBXStore=codeStore;
					erp.Util.combxStore=me.clone(codeStore);
				}
			});
			 ...
			 * 基础数据配置初始化
			 */
			var CodeStore=me.createStoreFactory('erp.setup.store.Codes',false,{storeId:'Code'});
			me.CodeConfigLoad();
			me.test();
            DataManager.lookup('code_config').load();
            ...
            * 2015-03-29
            **/
			/*if (!Ext.isEmpty(erp.UInfo.currentUser.accountMap))
				var czy_gh=erp.Util.currentUser.accountMap[0].ref_u_id
			else if(erp.UInfo.currentUser.isAdmin)
				var czy_gh='000';
            if (czy_gh){
	           //根据员工ID获取销售类别
	            Ext.Ajax.request({
	            	url:'crm/sellking.act?method=getSellkindList',
	            	params:{czy_gh:czy_gh},
	            	success:function(resp){
	            		var text=Ext.decode(resp.responseText);
	            		var sellKindStore=me.createStoreFactory("erp.crm.master.basicData.sellkind.store.Sellkind", false);
			            sellKindStore.storeId=erp.DataConst.SELLKIND_STOREID;
		            	sellKindStore.loadData(text.data);
			            me.setStoreToStoreManager(sellKindStore);
	            	}
	            });
            	//根据员工ID获取计划类别
		...
		me.CodeConfigLoad=function(){
			var me=this;
			var recs=erp.DataConst.Config.getRange();
			Ext.each(recs,function(rec){
...
//			}
    ...
    erp.DataUtil= erp.util.data.DataUtil = new this();
    erp.DataUtil.init();
});
