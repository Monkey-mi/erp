//供应商基本信息页面
Ext.define('erp.supplierAccess.view.SupplierBaseShow',{
	extend:'erp.ux.Panel',
	alias:'widget.SupplierBaseShow',
	requires:[
			  'erp.supplierAccess.view.SupplierBusShow',
			  'erp.supplierManager.store.AppInvoiceTitle',
			  'erp.view.master.purchaseDetail.store.MainUnit',
			  'erp.util.data.OutSideAreaData'
			],
	overflowY: 'auto',
	//height:800,
//	isAdd:false,
	...
	initComponent:function(){
		...
		Ext.apply(me,{
			...
			items:[{
    			...
    			items:[{
    				xtype:'fieldset',
    				title:'<span style="color:#008cd6">基本信息1</span>',
					collapsible: true,
					padding:4,
					layout:'column',
					defaults : {
...
//					},
			...
			,{//业务信息
				...
				//minHeight:'150',
				flex:1,
				itemId:'SupplierBusShow',
				xtype:'SupplierBusShow'
	...
	loadBaseData:function(rec){
		...
		var outSideArea = erp.OutSideAreaData.OutSideArea;
		var provinceNum = Math.floor(parseInt(contact_addr_code)/10000)*10000;
		var provinceName = outSideArea[86][provinceNum]; //省名称		
	    var cityNum = Math.floor(parseInt(contact_addr_code)/100)*100;
	    var cityName = outSideArea[provinceNum][cityNum];//市名称
	    var countryNum = contact_addr_code;
	    var countryName = outSideArea[cityNum][countryNum];//地区名称
		var contact_addr_all = provinceName + ',' + cityName + ',' + countryName;
		rec.set('contact_addr_all',contact_addr_all);
		me.down('#PanelBaseInfo').loadRecord(rec);
		//加载业务数据
		me.down('#SupplierBusShow').loadbusData(rec);
	}
});