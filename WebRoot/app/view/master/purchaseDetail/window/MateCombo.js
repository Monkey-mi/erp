 ...
 * 材料选择帮助窗口
 */
Ext.define('erp.view.master.purchaseDetail.window.MateCombo', {
	...
	initComponent : function() {
		...
		me.field=me.field||{};
		console.log(me.field.edtMaterialInspection);
		var val=me.field.getValue();
		if(val!=null && me.field.edtMaterialInspection!='edtMaterialInspection'){
			me.store.proxy.extraParams.xsbjsearch=val.replace(/\s+/g,"");
		}else{
			me.store.proxy.extraParams.s_spbj=1;
		}
		/*if(me.edtMaterialInspection){
			delete me.store.proxy.extraParams.xsbjsearch;
		}*/
		me.store.load();
		Ext.apply(this, {
					items : [{
	...
	forSetGrid : function(rec,recs) {
		...
		srec.set('fzsl', contractDetail.fzsl);
