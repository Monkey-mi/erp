Ext.define('erp.enquiryQuote.enquiry.controller.EnquiryCtrl', {
	extend : 'Ext.app.Controller',
	requires : [
				'erp.ux.PagingBar'
				,'Ext.ux.TreePicker'
				,'erp.enquiryQuote.enquiry.store.Enquiry'
				,'erp.enquiryQuote.enquiry.store.EnquiryDetail'
				,'erp.enquiryQuote.enquiry.store.TempMaterialClass'
				,'erp.basicdata.materialClass.store.MaterialClassTree'
				,'erp.enquiryQuote.enquiry.model.QueryParam'
				,'erp.enquiryQuote.enquiry.store.EnquiryDistribution'
				,'erp.enquiryQuote.enquiry.store.SupplierFile'
				,'erp.enquiryQuote.enquiry.store.TempQuotation'
				,'erp.enquiryQuote.enquiry.store.TempQuotationDetail'
				],
	views:[	'erp.ux.ComboxTree',
			'erp.enquiryQuote.enquiry.view.EnquiryManager',
			'erp.enquiryQuote.enquiry.view.EnquiryMaintain',
			'erp.enquiryQuote.enquiry.view.UpEnquiryFile'
			,'erp.enquiryQuote.enquiry.view.EnquiryQuery'
			,'erp.enquiryQuote.enquiry.view.SupplierWin'
		],
	refs:[
		...
		me.control({
			'enquiryManager':{
				afterrender:function(cmp){
					...
					me.panel.loadMain();
					me.query_rec=Ext.create('erp.enquiryQuote.enquiry.model.QueryParam');
		...
		switch(btn.itemId){
			case 'BTN_SAVE':
				...
				if(form.getForm().isValid()&&form.getForm().isDirty()){
					if(enquiry_item!=null && erp.Util.gettextlength(enquiry_item)>200){
			...
			case 'btn_enquirydetail_add':
				...
				var newrec=Ext.create('erp.enquiryQuote.enquiry.model.EnquiryDetail',{
					item_order:maxorder
				});
	...
	doMaintainAction:function(type){
		...
		switch(type){
			case erp.Const.FUNC_ITEMID_BTN_ADD:
					...
					isEdit=false;
					var today=new Date();
					rec=Ext.create('erp.enquiryQuote.enquiry.model.Enquiry',{
						enquiry_date:today,
						enquiry_person:erp.Util.currentUser.userInfo.name,
						enquiry_status:0,
						company_id:10000,
						cpyname_cn:'泰普森休闲用品有限公司'
					});
					break;
	...
	checkStatusSame:function(sel_recs,idarray,statusarray){
		var me=this;
		var json=erp.Const.callServiceMethodSync('enquiry/enquiry.do?method=checkStatusSame',{
	...
	dolockAction:function(){
		...
		Ext.Msg.confirm("提示",alertmsg,function(btn){
			if (btn=="yes")
			{	
				erp.Const.callServiceMethodSync('enquiry/enquiry.do?method=updateStatus',{
					idarray:idarray.join(','),
		...
		switch(btn.itemId){
			case 'BTN_SAVE':
				...
				for(var i=0;i<recs.length;i++){
					var newrec=Ext.create('erp.enquiryQuote.enquiry.model.EnquiryDistribution',{
	...
	hasQuote:function(idarray){
		var me=this;
		var json=erp.Const.callServiceMethodSync('quote/quotation.do?method=hasQuote',{
			idarray:idarray.join(',')
		});
		json=Ext.decode(json);
		return json.result;
