Ext.define('erp.enquiryQuote.quote.view.QuoteManager',{
	extend:'erp.ux.Panel',
	...
	initComponent : function() {
		var me=this;
		me.cannotedit=true;
		me.enquirystore=Ext.create('erp.enquiryQuote.quote.store.Enquiry');
		...
		me.detailstore=Ext.create('erp.enquiryQuote.quote.store.EnquiryDetail');
		Ext.apply(me.detailstore.proxy.extraParams,{usePaging:false});
		me.quotationstore=Ext.create('erp.enquiryQuote.quote.store.Quotation');
		Ext.apply(me.quotationstore.proxy.extraParams,{usePaging:false});
		me.quotationDetailStore=Ext.create('erp.enquiryQuote.quote.store.QuotationDetail');
...
//				            	return '<div style="color:red">未上传</div>';
	...
	openEditView:function(enquiryDetail_rec){
		var me=this;
		//加载数据
		var company_id=erp.UInfo.currentUser.supplier.get('company_id');
		var cpyname_cn=erp.UInfo.currentUser.supplier.get('cpyname_cn');
		me.quotationstore.load({
			...
			callback: function(records, operation, success) {
				...
				if(records.length==0){
					rec=Ext.create('erp.enquiryQuote.quote.model.Quotation',{
